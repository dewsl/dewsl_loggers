...
def comport_setup():
    ...
    while True:
        ...
        if counter > 60:
            ...
            raise UnrespondingDevice("No device detected.")
        counter += 1
    print(comport)
    return comport
...
def set_gsmpowmode(gpm):
    #W...C...gpm
    print("\nStart process: Updating GSM power mode")
    waitread("W", False, "Enter C to change:", 10, 1)
    swrite("C", 1)
...
class UnrespondingDevice(Exception):
    pass
...
def check_csv_integrity(row):
    if len(row) == 3:
        if row[1].strip() in ['0', '1']:
            if row[0].lower().strip() in ['realtimeclock', 'servernetwork', 'gsmpowmode', 'loggerversion', 'sendingtime', 'loggernames', 'sensorcommand', 'mcupassword']:
                ...
                if row[0].lower().strip() == 'servernetwork':
                    if ( row[2].upper().strip() in ['GLOBE', 'SMART'] ):
                        ...
                        return False
                if row[0].lower().strip() == 'gsmpowmode':
                    try:
                        int(row[2].strip())
                        pass
                    except Exception as e:
                        print(e, "Check gsmpowmode value.")
                        return False
...
def main():
    ...
    try:
        ...
        if conf['gsmpowmode'][0]:
            set_gsmpowmode(conf['gsmpowmode'][1])
