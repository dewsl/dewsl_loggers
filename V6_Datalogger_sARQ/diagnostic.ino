...
void initINA219() {
  ...
  else Serial.println("INA219 module ready");
}
void readINA219() {     
...
float readINA219VoltageCurrent (byte paramIndex) {                  //  Voltage is separated from current because these are times these are needed separately
  ...
  if (paramIndex == 0) paramHolder = INA219Module.getBusVoltage();  //  index 0 returns voltage
...
void diagnosticCheck(uint8_t mIndex) {
  switch (mIndex) {
    ...
    default: debugPrintln("Invalid diagnostic check index");
  }
}