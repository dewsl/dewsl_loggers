...
//ublox global variables
uint8_t *rtcm = (uint8_t *)malloc(sizeof(uint8_t) * 1000);
uint8_t *received_ublox = (uint8_t *)malloc(sizeof(uint8_t) * 400);
//rover variables
uint8_t *buf_total = (uint8_t *)malloc(sizeof(uint8_t) * 1000);
uint8_t *received_rover = (uint8_t *)malloc(sizeof(uint8_t) * 251);
//max retry for getting rtcm data
#define MAX_RETRY_BASE 200
#define MAX_RETRY_ROVER 30
...
int len1_rover, len2_rover;
int len_rover = 0, len_total_rover = 0;
long lastTime = millis();
long timer2 = millis();
bool ublox_flag = true;
...
char firmwareVersion[9] = "21.11.09"; //year . month . date
...
*/
  ...
  //initialize UBLOX
  /*
  if (get_logger_version() == 13 || get_logger_version() == 14)
  {
    Serial.println("initializing UBLOX . . . ");
  ...
  if (OperationFlag)
  {
    ...
    */
    ...
    else if (get_logger_version() == 14)
    {
      ...
      Serial.println("Read UBLOX data");
      read_ublox_data();
      delay_millis(1000);
