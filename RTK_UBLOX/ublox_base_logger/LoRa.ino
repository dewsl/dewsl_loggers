        ...
        if (rf95.available())
        {
            ...
            if (rf95.recv(buf, &len2))
            {
                ...
                { /*NOT LoRa: 0, 2, 6, 7*/
                    ...
                    if (mode == 1 || mode == 3 || mode == 4 || mode == 5 || mode == 12) //LoRa mode only
                    {
...
void send_thru_lora_base(uint8_t *radiopacket, int len)
{
    ...
    Serial.println("Sending to rf95_server -from Base");
...
int read_lora_data()
{
    ...
    if (rf95.waitAvailableTimeout(20000))
    {
        ...
        if (rf95.recv(buf, &len))
        {
            ...
            for (i = 0; i < len; ++i)
            {
                received_ublox[i] = buf[i];
            }
            //received from tx
            received_ublox[i] = (uint8_t)'\0';
...
int read_lora_data_rover(int start)
{
    uint8_t buf[RH_RF95_MAX_MESSAGE_LEN];
    uint8_t len = sizeof(buf);
    ...
    if (rf95.waitAvailableTimeout(3000))
    {
        ...
        if (rf95.recv(buf, &len))
        {
            ...
            else if (String((char *)buf).indexOf("*rtcm$") < 0)
            {
                return 0;
            }
            ...
            for (i = 6; i < len; ++i)
            {
                received_rover[i-6] = buf[i];
                buf_total[i -6 + start] = buf[i];

                if (received_rover[i-6] == 211 && i != 0)
                {
                    //Serial.println();
                    c = 0;
                }
...
void send_thru_lora_rover(char *radiopacket)
{
    ...
    Serial.println("Sending to rf95_server -from Rover");
    // Send a message to rf95_server
...
void read_lora_data_only(uint8_t mode)
{
    if (rf95.available())
    {
        if (rf95.recv(buf, &len))
