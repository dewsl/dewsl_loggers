...
//int *calib_data = (int *)malloc(sizeof(int) * 10);
//int *ave_data = (int *)malloc(sizeof(int) * 10);
...
char *read_IMU_data()
{
  ...
  strncpy((IMUdataToSend), "MADNA*0:", (20));
  ...
  for (int i = 0; i < 10; i++)
  {
    ...
    //if ((i+1)%3 == 0) strncat(IMUdataToSend, ";", 1);
    //else 
...
void setup(void) {
  ...
  pinMode(IMUSW, OUTPUT);
  pinMode(IMU_B, OUTPUT);
  digitalWrite(IMUSW, LOW);
  digitalWrite(IMU_B, LOW);
...
void loop() {
  ...
  print_data();
  read_IMU_data();
  // Serial.println(get_data_from_uart());
  // off_IMU();
  delay(10000);
...
void on_IMU()
{
  ...
  digitalWrite(IMU_B, HIGH);
  digitalWrite(IMUSW, HIGH);
  delay(1000);
  init_IMU();
  delay(2000);
